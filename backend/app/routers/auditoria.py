...
async def audit_image(
    ...
    current_user: UserResponse = Depends(require_role([UserRole.APROBADOR_B, UserRole.ADMIN]))
):
    supabase = get_supabase()
    
    contenido_response = supabase.table("contenido").select("*").eq("id", contenido_id).execute()
    ...
    auditoria_data = {
        ...
        "imagen_url": f"data:{image.content_type};base64,{image_base64[:100]}",
        "resultado": {
...
async def get_auditorias_by_contenido(
    ...
    supabase = get_supabase()
    response = supabase.table("auditorias").select("*").eq("contenido_id", contenido_id).execute()
...
async def list_auditorias(
    ...
    supabase = get_supabase()
    response = supabase.table("auditorias").select("*").order("created_at", desc=True).limit(limit).execute()
